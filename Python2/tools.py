...
def extract_qa_pairs(note_body: str) -> List[Tuple[str, str]]:
    ...
    Improvement: handles wrapped multi-line definitions (common in PDF-extracted text),
    by attaching continuation lines to the previous answer when it looks incomplete.
    """
    ...
    lines = [_strip_bullet(ln) for ln in raw_lines]
    ...
    for ln in lines:
        low = ln.lower()
        if low.startswith("q:"):
            pending_q = ln[2:].strip()
            continue
        if low.startswith("a:") and pending_q:
            a = ln[2:].strip()
            if pending_q and a:
    ...
    # --- separator mode (with continuation lines) ---
    i = 0
    while i < len(lines):
        l = lines[i]
        low = l.lower()
        if low.startswith("q:") or low.startswith("a:"):
            i += 1
            continue
        ...
        def _looks_like_new_qa(x: str) -> bool:
            if not x:
                return False
            xl = x.lower()
            if xl.startswith("q:") or xl.startswith("a:"):
                return True
            for pat2 in _SEP_PATTERNS:
                if re.search(pat2, x):
                    return True
            return False

        def _should_attach(ans: str, nxt: str) -> bool:
            if not ans or not nxt:
                return False
            if ans.rstrip().endswith((",", ";", "-", "–", "—")):
                return True
            if len(ans) < 60:
                return True
            if not ans.rstrip().endswith((".", "!", "?")) and nxt[:1].islower():
                return True
            return False
        ...
        while j < len(lines):
            nxt = lines[j]
            if _looks_like_new_qa(nxt):
                break
            if _should_attach(right, nxt):
                right = (right + " " + nxt).strip()
                j += 1
                continue
            break

        if left and right and len(left) <= 180 and len(right) <= 4000:
