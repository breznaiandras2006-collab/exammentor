...
from tools import (
    ...
    # Extras
    add_text_watermark_pdf_bytes,
    add_page_numbers_pdf_bytes,
    images_to_pdf_bytes,
    pdf_to_images_zip_bytes,
    compare_pdfs_text_summary,
    encrypt_pdf_bytes,
    strip_pdf_metadata_bytes,
    # Study
    extract_qa_pairs,
)
...
def _store_bytes_as_document(
    ...
    *,
    ...
    """Save arbitrary bytes into uploads/ + insert into documents table."""
    safe_orig = safe_filename(original_name, fallback=original_name or "file.bin")
    raw = (safe_orig + str(os.urandom(8))).encode("utf-8", "ignore")
    h = hashlib.sha256(raw).hexdigest()[:24]
    stored_name = f"{h}_{safe_orig}"
...
def _should_inline(media_type: str) -> bool:
...
def document_file(doc_id: int):
    ...
    original = doc.get("original_name") or fp.name
    media_type = _guess_media_type(original)
    inline = _should_inline(media_type)
    dispo = "inline" if inline else "attachment"
    return FileResponse(
        str(fp),
        media_type=media_type,
        filename=original,
        headers={"Content-Disposition": f'{dispo}; filename="{original}"'},
    )
...
def _render_pdf_tools(
    ...
    *,
    ...
    docs_all = list_documents()
    docs_pdf = [d for d in docs_all if (d.get("type") or d.get("doc_type") or "pdf") == "pdf"]
    ctx = {
        "request": request,
        "docs": docs_pdf,
        "result": result,
...
@app.post("/pdf-tools/watermark", response_class=HTMLResponse)
def pdf_tools_watermark(
    request: Request,
    doc_id: int = Form(...),
    watermark_text: str = Form(""),
    font_size: int = Form(46),
    ...
    if not doc:
        return _render_pdf_tools(
            ...
            result="❌ Nem találom ezt a dokumentumot.",
            result_kind="error",
            form={"wm_doc_id": doc_id, "wm_text": watermark_text},
        )
    ...
    try:
        out = add_text_watermark_pdf_bytes(
            fp.read_bytes(),
            watermark_text,
    ...
    original_out = f"watermark_{doc.get('original_name','document.pdf')}"
    title_out = f"Watermark — {doc.get('title') or doc.get('original_name','PDF')}"
    new_id = _store_pdf_bytes_as_document(out, title=title_out, original_name=original_out, language=doc.get("language") or "auto")
    new_doc = get_document(new_id)

    return _render_pdf_tools(
        request,
        result="✅ Watermark hozzáadva (új dokumentum készült).",
        result_kind="success",
        created_docs=[new_doc] if new_doc else [],
        form={
            "wm_doc_id": doc_id,
...
@app.post("/pdf-tools/page-numbers", response_class=HTMLResponse)
def pdf_tools_page_numbers(
    request: Request,
    doc_id: int = Form(...),
    start_at: int = Form(1),
    template: str = Form("{n}/{total}"),
    ...
    try:
        out = add_page_numbers_pdf_bytes(
            fp.read_bytes(),
            start_at=int(start_at),
            template=(template or "{n}/{total}"),
            font_size=int(font_size),
            y=int(y),
            opacity=float(opacity),
        )
    ...
    original_out = f"pagenum_{doc.get('original_name','document.pdf')}"
    title_out = f"Page numbers — {doc.get('title') or doc.get('original_name','PDF')}"
    new_id = _store_pdf_bytes_as_document(out, title=title_out, original_name=original_out, language=doc.get("language") or "auto")
    new_doc = get_document(new_id)

    return _render_pdf_tools(
        request,
        result="✅ Oldalszámozás kész (új dokumentum készült).",
        result_kind="success",
        created_docs=[new_doc] if new_doc else [],
        form={
            "pn_doc_id": doc_id,
...
@app.post("/pdf-tools/encrypt", response_class=HTMLResponse)
def pdf_tools_encrypt(
    request: Request,
    doc_id: int = Form(...),
    password: str = Form(""),
):
    ...
    if not doc:
        return _render_pdf_tools(
            ...
            result="❌ Nem találom ezt a dokumentumot.",
            result_kind="error",
            form={"enc_doc_id": doc_id, "enc_pw": password},
        )
    ...
    try:
        out = encrypt_pdf_bytes(fp.read_bytes(), password=password)
    except Exception as e:
        return _render_pdf_tools(
            request,
            status_code=400,
            result=f"❌ Titkosítás hiba: {e}",
            result_kind="error",
            form={"enc_doc_id": doc_id, "enc_pw": password},
        )

    original_out = f"encrypted_{doc.get('original_name','document.pdf')}"
    title_out = f"Encrypted — {doc.get('title') or doc.get('original_name','PDF')}"
    new_id = _store_pdf_bytes_as_document(out, title=title_out, original_name=original_out, language=doc.get("language") or "auto")
    new_doc = get_document(new_id)

    return _render_pdf_tools(
        request,
        result="✅ Jelszavas PDF elkészült (új dokumentum).",
        result_kind="success",
        created_docs=[new_doc] if new_doc else [],
        form={"enc_doc_id": doc_id, "enc_pw": password},
    )
...
@app.post("/pdf-tools/strip-metadata", response_class=HTMLResponse)
def pdf_tools_strip_metadata(
    request: Request,
    doc_id: int = Form(...),
):
    doc = get_document(doc_id)
    ...
    try:
        out = strip_pdf_metadata_bytes(fp.read_bytes())
    except Exception as e:
        return _render_pdf_tools(
            request,
            status_code=400,
            result=f"❌ Metaadat törlés hiba: {e}",
            result_kind="error",
            form={"meta_doc_id": doc_id},
        )

    original_out = f"no_metadata_{doc.get('original_name','document.pdf')}"
    title_out = f"No metadata — {doc.get('title') or doc.get('original_name','PDF')}"
    new_id = _store_pdf_bytes_as_document(out, title=title_out, original_name=original_out, language=doc.get("language") or "auto")
    new_doc = get_document(new_id)

    return _render_pdf_tools(
        request,
        result="✅ Metaadatok eltávolítva (új dokumentum).",
        result_kind="success",
        created_docs=[new_doc] if new_doc else [],
        form={"meta_doc_id": doc_id},
    )
...
async def pdf_tools_images_to_pdf(
    request: Request,
    images: list[UploadFile] = File(...),
    title: str = Form("Images PDF"),
):
    ...
    for f in images:
        ...
        if not bts:
            continue
        files.append((name, bts))

    try:
        out = images_to_pdf_bytes(files)
    except Exception as e:
        return _render_pdf_tools(
            request,
            status_code=400,
            result=f"❌ Képek→PDF hiba: {e}",
            result_kind="error",
            form={"img2pdf_title": title},
        )

    title2 = (title or "Images PDF").strip() or "Images PDF"
    original_out = safe_filename(f"{title2}.pdf", "images.pdf")
    new_id = _store_pdf_bytes_as_document(out, title=title2, original_name=original_out, language="auto")
    new_doc = get_document(new_id)

    return _render_pdf_tools(
        request,
        result=f"✅ Képekből PDF készült ({len(files)} kép).",
        result_kind="success",
        created_docs=[new_doc] if new_doc else [],
        form={"img2pdf_title": title},
    )
...
def pdf_tools_pdf_to_images(
    ...
    if not doc:
        return _render_pdf_tools(
            ...
            result="❌ Nem találom ezt a dokumentumot.",
            result_kind="error",
            form={"p2i_doc_id": doc_id, "p2i_dpi": dpi, "p2i_fmt": fmt},
        )
    ...
    try:
        zip_bytes = pdf_to_images_zip_bytes(fp.read_bytes(), dpi=int(dpi), fmt=(fmt or "png"))
    except Exception as e:
        return _render_pdf_tools(
            request,
            status_code=400,
            result=f"❌ PDF→képek hiba: {e}",
            result_kind="error",
            form={"p2i_doc_id": doc_id, "p2i_dpi": dpi, "p2i_fmt": fmt},
        )

    base = Path(doc.get("original_name") or "document.pdf").stem
    ext = "png" if (fmt or "png").lower() == "png" else "jpg"
    original_out = f"{base}_pages_{int(dpi)}dpi.{ext}.zip"
    title_out = f"PDF→Images ZIP — {doc.get('title') or base}"
    new_id = _store_bytes_as_document(
        zip_bytes,
        title=title_out,
    ...
    return _render_pdf_tools(
        request,
        result="✅ PDF oldalai képként exportálva (ZIP kész).",
        result_kind="success",
        created_docs=[new_doc] if new_doc else [],
        form={"p2i_doc_id": doc_id, "p2i_dpi": dpi, "p2i_fmt": fmt},
    )
...
@app.post("/pdf-tools/compare", response_class=HTMLResponse)
def pdf_tools_compare(
    request: Request,
    doc_a: int = Form(...),
    doc_b: int = Form(...),
):
    a = get_document(doc_a)
    b = get_document(doc_b)
    if not a or not b:
        return _render_pdf_tools(
            request,
            status_code=404,
            result="❌ A megadott dokumentumok közül valamelyik nem található.",
            result_kind="error",
            form={"cmp_a": doc_a, "cmp_b": doc_b},
        )

    fa = UPLOAD_DIR / (a.get("stored_name") or "")
    fb = UPLOAD_DIR / (b.get("stored_name") or "")
    if not fa.exists() or not fb.exists():
        return _render_pdf_tools(
            request,
            status_code=404,
            result="❌ Hiányzó uploads fájl (A vagy B).",
            result_kind="error",
            form={"cmp_a": doc_a, "cmp_b": doc_b},
        )

    try:
        summary = compare_pdfs_text_summary(fa.read_bytes(), fb.read_bytes())
    except Exception as e:
        return _render_pdf_tools(
            request,
            status_code=400,
            result=f"❌ Compare hiba: {e}",
            result_kind="error",
            form={"cmp_a": doc_a, "cmp_b": doc_b},
        )
