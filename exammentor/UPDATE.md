...
## PDF Tools v3 (extras)
- ✅ Text watermark (font size, rotation, opacity)
- ✅ Page numbering (template: `{n}` / `{total}`)
- ✅ Images → PDF (multi-upload)
- ✅ PDF → Images (ZIP export: PNG/JPG, DPI)
- ✅ PDF compare (text-based diff summary)
- ✅ PDF encryption (user password)
- ✅ Strip metadata (rewrite)

Notes:
- PDF→Images requires PyMuPDF (`PyMuPDF==1.24.9`)
- Images→PDF requires Pillow (`Pillow==10.4.0`)
